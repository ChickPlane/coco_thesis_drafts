...
\end{figure}
%\fi
 Fig\ref{fig:threeagent1} shows the routes of agents when $a=3$.
\begin{figure} [H]
  \centering 
...
The routes of agents are showed in Fig.\ref{fig:caseone} where a blue node indicates that there is one agent residing here; a red node indicates that there are two agents residing here; a yellow node indicate that there are three agents residing here. 
...
\end{figure}
After we transform the problem, then we can use the BVD-qG to solve the PBVD-qG. 
...
We now compare the time cost, the number of movement and the casualties with different number of agents we employ. (Assuming that we are in a $d_1\times \ldots \times d_q$ q-dimensional grid)
