...
Agents and nodes are anonymous, but for the purpose of easier description,  we refer to the nodes as $x_0$,  $x_1$ \dots.
...
\noindent{\bf Overview of the NotifyingMovingAgents phase}
...
We now describe how  the agents and the $CA$ move in coordination  in the example of   $C_n(1, 2, 7, 11)$  (see Figure \ref{fig:T29}). 
\begin{figure}[H]
  \centering  
...
At time $T_{noti\_5^{(2)}}$, when the $CA$ arrives at node $x_{77}$, it knows that it just pass its $Ending\,Flag$ so it moves along the longest chord anticlockwise to its $Beginning\,Flag$ (node $x_{66}$ at $T_{noti\_5^{(3)}})$.
\begin{figure}[H]
  \centering  
...
An alternative strategy, which allows a faster decontamination at the expense of a larger number of agents, is to  eliminate  the BVs concurrently.
...
%\section{Analysis}
 ...
 %\begin{theorem}
%Let  ${\cal C}$ be a chordal ring  $C_n(d_1, \dots, d_{k-1}, d_k)$.  
%Regardless of the position of the BV, the number of $Keep\,Moving$ agents in ${\cal C}$ is constant.
%\end{theorem}
%\begin{proof}
%\color{blue}
%Let us call  {\em   Go-Column}, the column in which the agents keep going when an explorer encounters a virus,
%and  {\em  Stop-Column}, the column in which the agents stop moving when an explorer encounters a virus.
%Let  the    $k$  columns in the matrix be  $M={c_0, \dots, c_{k-1}}$\\
%Let  the original BV be in  $node_V$, then it should be in the column $c_{V\,mod\,k}$.\\
%A column $c_S$ is a Stop-Column  if and only if $\exists i\in [1,k]$ such  that $(V+d_i)\,mod\,k=S$ or $(V-d_i)\,mod\,k=S$.\\
%If the explorer encounters the virus at the position $node_{V+a}$ instead of $node_V$, since $(V+d_i+a)\,mod\,k=(S+a)\,mod\,k$ or $(V-d_i+a)\,mod\,k=(S+a)\,mod\,k$, the column $c_{(S+a)\,mod\,d}$ is  a Stop-Column.\\
%In another words, if $c_S$ is a Stop-Column when the virus position is $V$, then $c_{(S+a)\,mod\,k}$ is a Stop-Column when the virus position is $V+a$.\\
%We assume we have two different Stop-Columns $c_x$ and $c_y$ ($0\leq x\leq d-1$, $0\leq x\leq d-1$) when the black virus's position is $V$.  So when the black virus's position is $V+a$, the Stop-Columns are $c_{(x+a)\,mod\,k}$ and $c_{(y+a)\,mod\,k}$. It is obvious that $\forall a$, if $x\neq y$, then $(x+a)\,mod\,d\neq(y+a)\,mod\,d$. This means that we still have two different Stop-Columns.
%So that the number of stop column does not decrease, which means that given a fixed chordal ring $C$ and the number of agent keeping moving when the BV is in V (V can be any position), then the number of agent keeping moving when the BV is in other position does not decrease.\\  
%Let us assume that the number of agents keeping moving in different cases when the longest chord remains the same is different and let us denote the minimum number of Go-Columns among them by $N_{min}$ and  the maximum number of Go-Columns among them by $N_{max}$, then according to our conclusion,  $N_{min}\geq N_{max}$, which means that $N_{min}= N_{max}$. In other words, the number of agents keeping moving is constant.  \color{black}
%\end{proof}
