...
if ($p \leq i+1 < k$)\\
\>   $N_{ex}(v_{i+1}) = \{ v_{i},v_{i+1-p} \}$ \\
\>  1 $SH$ is deployed to protect $v_{i+1-p}$\\ 

if ($k \leq i+1 < n-k$)\\
\>   $N_{ex}(v_{i+1}) = \{ v_{i},v_{i+1-p},v_{i+1-k} \}$ \\
\>  2 $SH$s are deployed to protect $v_{i+1-p},v_{i+1-k}$\\ 

if ($n-k \leq i+1 < n-p$)\\
\>   $N_{ex}(v_{i+1}) = \{ v_{i},v_{i+1-p},v_{i+1-k},v_{i+1+k} \}$ \\
\>  3 $SH$s are deployed to protect $v_{i+1-p},v_{i+1-k},v_{i+1+k}$\\ 

if ($n-p \leq i+1 < n-1$)\\
\>   $N_{ex}(v_{i+1}) = \{ v_{i},v_{i+1-p},v_{i+1-k},v_{i+1+k},v_{i+1+p} \}$ \\
\>  $4$ $SH$s are deployed to protect $v_{i+1-p},v_{i+1-k},v_{i+1+k},v_{i+1+p}$\\ 


if ($i= n-2$)\\
\>   $N_{ex}(v_{i+1}) =  \{ v_{i},v_{i+1-p},v_{i+1-k},v_{i+1+k},v_{i+1+p},v_{0} \}$ \\
\>  $5$ $SH$s are deployed to protect $v_{i+1-p},v_{i+1-k},v_{i+1+k},v_{i+1+p},v_{0}$\\ \\
 EA moves to $v_{i+1}$.\\ 
 \end{tabbing}
\end{minipage}
}
