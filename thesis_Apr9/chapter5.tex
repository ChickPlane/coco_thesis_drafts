...
% As for all the other topologies,  the exploration team locates the  {\it black virus}, then surrounding agents are sent to surround the newly created viruses  and to eliminate them in order to disinfect  the whole topology.

%We investigate several variants of the main strategy, depending on the routing procedure followed to surround the new triggered viruses.
%We evaluate the complexity 
% by considering the  overall number of agents employed, $Size(C)$, the total number of black viruses, $Spread(C)$, and the number of moves, $Move(C)$.
...
\begin{center}
 ...
 \hline
   Triple Loop      & Casualties & Agents & Moves    \\
 &W.C&W.C&Corresponding\\
 \hline
 Move-Optimal   &  ?  & ? & $?$  \\ %$4n-6$  \\ %3k-5 to reach node k-1 then phase 2=35 since it's %shortlychorded and s_area<k
 \hline
 Greedy   &  ? & ? &   $?$\\
 \hline
 Smart Greedy   &  ? & ? &   $?$ \\
...
The correctness of this phase stems from the fact that the exploring team follows the outer ring and that $EA$ always precedes $LEA$, so that the \bv is eventually detected in a monotone fashion. As we discussed in \ref{monotone_ph1},  monotonicity is achieved since the maximum number of $SH$s recruited from the beginning is five (i.e., as many as the number of neighbours in the worst case $v=n-1$).
 \begin{center}
\fbox{
\begin{minipage}{6cm}
...
if ($p \leq i < k$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-p} \}$ \\
\>  1 $SH$ is deployed to protect $v_{i-p}$\\ \\

if ($k \leq i < n-k$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-p},v_{i-k} \}$ \\
\>  2 $SH$s are deployed to protect $v_{i-p},v_{i-k}$\\ \\

if ($n-k \leq i < n-p$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-p},v_{i-k},v_{i+k} \}$ \\
\>  3 $SH$s are deployed to protect $v_{i-p},v_{i-k},v_{i+k}$\\ \\

if ($n-p \leq i < n-1$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-p},v_{i-k},v_{i+k},v_{i+p} \}$ \\
\>  $4$ $SH$s are deployed to protect $v_{i-p},v_{i-k},v_{i+k},v_{i+p}$\\ \\


if ($i = n-1$)\\
\>   $N_{ex}(v_i) =  \{ v_{i-1},v_{i-p},v_{i-k},v_{i+k},v_{i+p},v_{i+1} \}$ \\
\>  $5$ $SH$s are deployed to protect $v_{i-p},v_{i-k},v_{i+k},v_{i+p},v_{i+1}$\\ \\
 EA moves to $v_{i+1}$.\\ 
 \end{tabbing}
\end{minipage}
}
...
if ($|S_{area}|<p$) ($$LEA$$ is covering $x_{-1}$) \\
  ...
  \tab   $N_{un}(x_0) =\{x_{1},x_{p},x_{k},x_{-p},x_{-k} \}$ \\ \\


 if ($p\leq |S_{area}|<k$) ($$LEA$$ and  $SH$  covering $x_{-1},x_{-p}$)\\ 
 
  \tab   $N_{un}(x_0) =\{x_{1},x_{k},x_{p},x_{-k} \}$ \\ \\

if  ($k\leq |S_{area}| <n-k$) ($$LEA$$ \= and  2$SH$s  covering $x_{-1},x_{-p},x_{-k}$) \\
  
  \tab  $N_{un}(x_0) =\{x_{1},x_{p},x_{k}\}$  \\ \\

if  ($n-k\leq |S_{area}| <n-p$) ($$LEA$$ \= and  $SH$s  covering $x_{-1},x_{-p},x_{-k},x_{k}$) \\

  \tab  $N_{un}(x_0) =\{x_{1},x_{p},\}$  \\ \\

if  ($n-p\leq |S_{area}| <n-1$)($$LEA$$ \= and  $SH$s  covering $x_{-1},x_{-p},x_{-k},x_{k},x_{p}$) \\

  \tab  $N_{un}(x_0) =\{x_{1}\}$  \\ \\


Else ($$LEA$$ \= and  $SH$s  covering $x_{-1},x_{-p},x_{-k},x_{k},x_{p},x_1$) \\
...
if ($2 \leq i < k$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-2} \}$ \\
\>  1 $SH$ has been deployed to protect $v_{i-2}$\\ \\

if ($k \leq i < n-k$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-2},v_{i-k} \}$ \\
\>  2 $SH$s have been deployed to protect $v_{i-2},v_{i-k}$\\ \\

if ($n-k \leq i < n-2$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-2},v_{i-k},v_{i+k} \}$ \\
\>  3 $SH$s have been deployed to protect $v_{i-2},v_{i-k},v_{i+k}$\\ \\

if ($ i = n-2$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-2},v_{i-k},v_{i+k},v_{i+2} \}$ \\
\>  $4$ $SH$s have been deployed to protect $v_{i-2},v_{i-k},v_{i+k},v_{i+2}$\\ \\


if ($i = n-1$)\\
\>   $N_{ex}(v_i) =  \{ v_{i-1},v_{i-2},v_{i-k},v_{i+k},v_{i+2},v_{i+1} \}$ \\
\>  $5$ $SH$s have been deployed to protect $v_{i-2},v_{i-k},v_{i+k},v_{i+2},v_{i+1}$\\ \\
 EA moves to $v_{i+1}$.\\ 
 \end{tabbing}
\end{minipage}
}
...
$x_{2k}$ &   $ x_{0} \xrightarrow {-1} x_{-1} \xrightarrow {+k} x_{k-1} \xrightarrow {+k} x_{2k-1} \xrightarrow {+1}  x_{2k}$\\
 ...
 Depending on the chord structure,  shorter paths could be devised in some cases. A detailed analysis of all the possible situations is carried out in the following discussion.
...
The exploring and shadowing phase is the same in all triple loop rings. The only difference is the location of $SH$s which depends on the chord structure.
...
if ($ i = k-1$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-k+1} \}$ \\
\>  1 $SH$ is deployed to protect $v_{i-k+1}$\\ \\

if ($k \leq i < n-k$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-k+1},v_{i-k} \}$ \\
\>  2 $SH$s are deployed to protect $v_{i-k+1},v_{i-k}$\\ \\

if ($ i = n-k$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-k+1},v_{i-k},v_{i+k} \}$ \\
\>  3 $SH$s are deployed to protect $v_{i-k+1},v_{i-k},v_{i+k}$\\ \\

if ($n-k+1\leq i < n-1$)\\
\>   $N_{ex}(v_i) = \{ v_{i-1},v_{i-k+1},v_{i-k},v_{i+k},v_{i+k-1} \}$ \\
\>  $4$ $SH$s are deployed to protect $v_{i-k+1},v_{i-k},v_{i+k},v_{i+k-1}$\\ \\


if ($i = n-1$)\\
\>   $N_{ex}(v_i) =  \{ v_{i-1},v_{i-k+1},v_{i-k},v_{i+k},v_{i+k-1},v_{i+1} \}$ \\
\>  $5$ $SH$s are deployed to protect $v_{i-k+1},v_{i-k},v_{i+k},v_{i+k-1},v_{i+1}$\\ \\
 EA moves to $v_{i+1}$.\\ 
 \end{tabbing}
\end{minipage}
}
...
Depending on the chord structure and the size of the safe area, it may be possible to devise shorter paths in some cases. A detailed analysis of all possible situations is provided in the following  section.
