...
\\
if ($k\leq j <n-k$)\\
\>   $N_{ex}(v_j) = \{ v_{j-1},v_{j-k} \}$ \\
\>  1 $SH$ is deployed to protect $v_{j-k}$ \\ \\

if ($n-k\leq j < n-1$)\\
\>   $N_{ex}(v_j) = \{ v_{j-1},v_{j-k},v_{j+k} \}$ \\
\>    2 $SH$s are deployed to protect $v_{j-k}$,$v_{j+k}$\\ \\

if ($j = n-1$)\\
\>   $N_{ex}(v_j) = \{ v_{j-1},v_{j-k},v_{j+k},v_{j+1} \}$ \\
\>    3 $SH$s are deployed to protect $v_{j-k}$,$v_{j+k}$,$v_{j+1}$ \\ \\
...
\hline
 ...
 \hline
 $x_{2k}$ & $[x_0, x_{-1}, x_{k-1}, x_{2k-1},x_{2k}]$ & $2$ \\ %(if $ \lceil{n \over k}\rceil>4$ )\\
...
\hline
\end{tabular} 
\captionof{table}{Greedy paths with \bvs when $\left\vert{S_{area}}\right\vert < k$ }
...
%
%It is interesting to notice, however, that it does produce the optimal paths
%only for a very particular chord structure.
%
%\begin{property}  
%The   Greedy algorithm is optimal in double loops with $BV$s only if $k \leq 5$ and $ \lceil{n \over k}\rceil>4$  .
%\end{property}  
%\begin{proof}  
%
%{\bf Not sure.}
...
\subsection{Smart Greedy Algorithm}
...
{\sc tie-break}
\begin{tabbing}
Agent $A$ at $v$  with destination $z$\\ \\
...
\tab  $next$ is $\in N(v)$ the has the neighbour with the minimum distance. 
...
\hline
\end{tabular} 
\captionof{table}{Smart greedy paths with \bvs when $\left\vert{S_{area}}\right\vert \ge k$ }
...
\hline
\end{tabular} 
\captionof{table}{Smart greedy paths with \bvs when $\left\vert{S_{area}}\right\vert < k$ }
...
\end{tabular}
\captionof{table}{Comparison between the complexity of the three strategies.}
\end{center}
...
\end{tabular}
\captionof{table}{The overall complexity of the two phases.}
\end{center}
...
\fbox{
\begin{minipage}{7.5cm}
{\sc Surrounding and Elimination} \\(Sequential)
  \begin{tabbing}
